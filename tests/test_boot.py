...
def test_boot_wires_a_serving_app_and_seeds_the_store(
    ...
    assert boot_line.splitlines()[0] == (
        ...
        f"data_dir_configured=True auth=on token_len={len(PRODUCTION_TOKEN)} storage=accepted"
    ), boot_line
    assert PRODUCTION_TOKEN not in boot_line, "the boot line printed the team token"
